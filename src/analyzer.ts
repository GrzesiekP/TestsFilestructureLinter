...
export class Analyzer {
    async analyzeProject(options: Partial < AnalyzerOptions > = {}): Promise < {
        results: AnalysisResult[];totalFiles: number
    } > {
        ...
        for (const testFile of testFiles) {
            ...
            const matchingSourceFiles = await this.findMatchingSourceFiles(sourceFiles, sourceFileName, mergedOptions.fileExtension);
        ...
        if (mergedOptions.validateMissingTests) {
            const missingTestResults = await this.analyzeMissingTests(sourceFiles, testFiles, mergedOptions);
    ...
    async findTestFiles(dir: string, extension: string, testFileSuffix: string, ignoreDirectories: string[] = [], ignoreFiles: string[] = [], testProjectSuffix: string = '.Tests'): Promise < string[] > {
        try {
    ...
    async findSourceFiles(dir: string, extension: string, ignoreDirectories: string[] = [], ignoreFiles: string[] = []): Promise < string[] > {
        try {
    ...
    async findMatchingSourceFiles(sourceFiles: string[], baseName: string, extension: string): Promise < string[] > {
    ...
    findFirstIncorrectSegment(actualPath: string, expectedPath: string, options: AnalyzerOptions): string | null {
    ...
    async analyzeMissingTests(sourceFiles: string[], testFiles: string[], options: AnalyzerOptions): Promise < AnalysisResult[] > {
        const results: AnalysisResult[] = [];
        const testFileMap = new Map < string,
            string > ();
